...
class PoseNet(nn.Module):
    ...
    def forward(self, img, pc, pred_masks, part_feat, part_cnt, equiv_edge_indices):
        ...
        center2, quat2 = self.refine_net(pc2, feat, part_cnt, equiv_edge_indices)
...
class Network(nn.Module):
    ...
    def forward(self, img, pc, ins_feat, part_cnt, equiv_edge_indices):
        '''
            input:
            img:            max_parts x 3 x H x W
            pc :            max_parts x N x 3
        ...
        with torch.no_grad():
            pred_masks, part_feat = self.mask_net(img, pc, ins_feat, part_cnt)        

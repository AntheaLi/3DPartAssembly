...
class DGCNN(nn.Module):
    def __init__(self, emb_dim=512, k = 20, dropout=0.5):
        ...
        self.conv5 = nn.Sequential(nn.Conv1d(512, emb_dim, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.linear1 = nn.Linear(2*emb_dim, 512, bias=False)
        self.bn6 = nn.BatchNorm1d(512)
        self.dp1 = nn.Dropout(p=dropout)
        self.linear2 = nn.Linear(512, emb_dim)
...
class Network(nn.Module):
    ...
    def __init__(self, conf, partleaf_cnt ):
        ...
        self.pointnet = DGCNN(emb_dim=conf.pointnet_emd_dim, k=10)

...
class Network(nn.Module):
    ...
    def forward(self, img, pc, sem_feat, ins_feat, box_size, part_cnt):
        ...
        t = 0
        for cnt in part_cnt:
            # i dont need to have the network predict the background mask, just have the current background 
            # to match the actual background
            # background_cond_feat = Variable(torch.zeros_like(cond_feat[0], dtype=torch.float), requires_grad = True)
            cur_cond_feat = cond_feat[t: t+cnt]
            conf_feat = torch.cat( [cur_cond_feat, background_cond_feat] , dim=0)
        ...
        for cnt in part_cnt:
            ...
            cur_mask = self.softmax( torch.cat( masks[t:t+cnt+1], 0))

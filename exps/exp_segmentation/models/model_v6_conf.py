...
class Network(nn.Module):
    ...
    def __init__(self, conf, partleaf_cnt ):
        ...
        self.max2d = nn.MaxPool2d((14, 14), stride=(1, 1))
        self.mlp4 = nn.Linear(self.dim_len + 512 + 256 , 256)
        self.mlp5 = nn.Linear(256 ,1)
    ...
    def forward(self, img, pc, sem_feat, ins_feat, box_size, part_cnt):
        ...
        conf = self.max2d(bottleneck).view(batch_size, 512)
        conf = torch.cat( [conf, cond_feat], dim=1) # batch size x (512+256+256)
        conf = torch.relu(self.mlp4(conf))
        conf = torch.sigmoid(self.mlp5(conf))

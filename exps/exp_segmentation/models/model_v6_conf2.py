...
class Network(nn.Module):
    ...
    def __init__(self, conf, partleaf_cnt ):
        ...
        if conf.use_semantics:
            self.dim_len = conf.pointnet_emd_dim + self.sem_dim + conf.ins_dim + self.box_dim
        ...
        self.max2d_bottleneck = nn.MaxPool2d((14, 14), stride=(1, 1))
        self.avg2d_mask = nn.AvgPool2d((4, 4), stride=(4, 4))
        self.conf_alpha = conf.conf_alpha
        self.mlp4 = nn.Linear(512 , 256)
        self.mlp5 = nn.Linear(56 * 56 ,256)
        self.conf_final = nn.Linear(256 + 256, 1)


    def forward(self, img, pc, sem_feat, ins_feat, box_size, part_cnt):
        '''
            input:
        ...
        if self.conf.use_semantics:
            cond_feat = torch.cat([pc_feat, sem_feat, box_size, ins_feat], dim=1)
        else:
        ...
        for cnt in part_cnt:
            cur_global_feat = global_part_feat[t:t+cnt].max(dim=0, keepdim=True)[0]
            cur_global_feat = torch.relu(self.mlp2(cur_global_feat)).repeat(cnt, 1)
            global_feat.append(cur_global_feat)
            t += cnt

        global_feat = torch.cat(global_feat, dim=0)

        cond_feat = torch.cat( (cond_feat, global_feat), dim=1)

        masks, bottleneck = self.unet(img, cond_feat)
        ...
        conf_bottleneck = self.max2d_bottleneck(bottleneck).view(batch_size, 512)
        conf_bottleneck = torch.relu(self.mlp4(conf_bottleneck))
        conf_mask = self.avg2d_mask(masks).view(batch_size, -1)
        conf_mask = torch.relu(self.mlp5(conf_mask))
        conf = torch.cat([conf_bottleneck, conf_mask], dim=1)
        conf = torch.sigmoid(self.conf_final(conf))

        return all_masks, conf
    ...
    def get_conf_loss(self, confidence, s_iou):
        
        loss_per_data = (confidence + s_iou).pow(2).sum(dim=1)

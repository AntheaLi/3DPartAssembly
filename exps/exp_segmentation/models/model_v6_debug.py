...
class Network(nn.Module):
    ...
    def __init__(self, conf, partleaf_cnt ):
        ...
        self.dim_len -= conf.pointnet_emd_dim
        self.unet = UNet(self.dim_len + 256, \
            in_channels=3, n_classes=1, padding=True, batch_norm=False)
    ...
    def forward(self, img, pc, sem_feat, ins_feat, box_size, part_cnt):
        ...
        if self.conf.use_semantics:
            cond_feat = torch.cat([sem_feat, box_size, ins_feat], dim=1)
        else:
            cond_feat = torch.cat([box_size, ins_feat], dim=1)

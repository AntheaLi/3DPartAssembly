...
def train(conf):
    ...
    if conf.load_ckpts_dir != "":
        print('Loading ckpt from ', os.path.join('logs', conf.load_ckpts_dir, 'ckpts'), conf.model_ckpts_epoch)
        _ = utils.load_checkpoint(
            models, model_names,
            os.path.join('logs', conf.load_ckpts_dir, 'ckpts'),
            epoch=conf.model_ckpts_epoch,
...
def forward(batch, data_features, network, conf, \
    ...
    pred_masks = network(input_img - 0.5, input_pts, input_sem_one_hot, input_ins_one_hot, input_total_part_cnt)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_ckpts_dir', type=str, default='', help='load ckpts directory')
    parser.add_argument('--model_ckpts_epoch', type=str, default='', help='load ckpts directory')

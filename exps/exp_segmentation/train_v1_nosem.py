...
def train(conf):
    ...
    data_features = ['img', 'pts','ins_one_hot' , 'total_parts_cnt' , 'similar_parts_cnt', 'mask' ,'shape_id', 'view_id']
...
def forward(batch, data_features, network, conf, \
    ...
    while total_part_cnt < 32 and batch_index < cur_batch_size:
        ...
        batch_index += 1
    input_img = input_img.to(conf.device); input_pts = input_pts.to(conf.device);  

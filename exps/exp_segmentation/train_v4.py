...
def train(conf):
    ...
    data_features = ['img', 'pts', 'sem_one_hot', 'ins_one_hot' , 'total_parts_cnt' , 'similar_parts_cnt', 'mask' ,'shape_id', 'view_id']
...
def forward(batch, data_features, network, conf, \
    ...
    pred_masks, pred_conf = network(input_img - 0.5, input_pts, input_sem_one_hot, input_ins_one_hot, input_total_part_cnt)
    
    # perform matching and calculate masks 
    mask_loss_per_data = []; mask_conf_loss_per_data=[]; t = 0; 
    ...
    # for mask_conf in mask_conf_loss_per_data:
    ...
    mask_conf_loss = mask_conf_loss_per_data.mean()

    # compute total loss
    total_loss = mask_loss * conf.loss_weight_mask + \
                mask_conf_loss * conf.loss_weight_mask_conf
    ...
    with torch.no_grad():
        ...
        if log_tb and tb_writer is not None:
            ...
            tb_writer.add_scalar('mask_conf_loss', mask_conf_loss.item(), step)
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                for i in range(batch_size):
                    ...
                    with open(os.path.join(info_dir, fn.replace('.png', '.txt')), 'w') as fout:
                        ...
                        fout.write('ins_one_hot: %s\n' % (str(input_ins_one_hot[i]))) 
                        fout.write('conf sore: %f\n' % ( mask_conf_loss_per_data[i]))   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_weight_mask_conf', type=float, default=1.0, help='loss weight')

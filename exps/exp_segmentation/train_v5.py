...
def train(conf):
    ...
    data_features = ['img', 'pts', 'sem_one_hot', 'ins_one_hot_new' , 'total_parts_cnt' , 'similar_parts_cnt', 'mask' ,'shape_id', 'view_id']
...
def forward(batch, data_features, network, conf, \
    ...
    input_ins_one_hot = batch[data_features.index('ins_one_hot_new')][0].squeeze(0)[:input_total_part_cnt]             # part_cnt x max_similar_parts
    ...
    while total_part_cnt < 32 and batch_index < cur_batch_size:
        ...
        input_ins_one_hot = torch.cat((input_ins_one_hot, batch[data_features.index('ins_one_hot_new')][batch_index].squeeze(0)[:cur_input_cnt]), dim=0)    # B x max_parts x max_similar_parts

...
def train(conf):
    ...
    data_features = ['img', 'pts', 'sem_one_hot', 'ins_one_hot' , 'box_size', 'total_parts_cnt' , 'similar_parts_cnt', 'mask_background' ,'shape_id', 'view_id']
...
def forward(batch, data_features, network, conf, \
    ...
    # prepare gt: 
    gt_mask = (batch[data_features.index('mask_background')][0].squeeze(0)[:input_total_part_cnt].to(conf.device),)  
    ...
    while total_part_cnt < 32 and batch_index < cur_batch_size:
        ...
        # prepare gt
        gt_mask = gt_mask + (batch[data_features.index('mask_background')][batch_index].squeeze(0)[:cur_input_cnt].to(conf.device), )
        batch_index += 1
    ...
    pred_masks = network(input_img - 0.5, input_pts, input_sem_one_hot, input_ins_one_hot, input_box_size, input_total_part_cnt)

...
def train(conf):
    ...
    data_features = ['img', 'pts', 'sem_one_hot', 'ins_one_hot' , 'box_size', 'total_parts_cnt' , 'similar_parts_cnt', 'mask' ,'shape_id', 'view_id']
    ...
    if not conf.no_console_log:
        header = '     Time    Epoch     Dataset    Iteration    Progress(%)       LR    MaskLoss     ConfLoss      TotalLoss'
...
def forward(batch, data_features, network, conf, \
    ...
    pred_masks, pred_conf = network(input_img - 0.5, input_pts, input_sem_one_hot, input_ins_one_hot, input_box_size, input_total_part_cnt)
    
    # perform matching and calculate masks 
    mask_loss_per_data = []; t = 0; mask_conf_loss_per_data = []
    matched_pred_mask_all = []; matched_gt_mask_all = []; matched_pred_mask_conf_all = [] 
    for i in range(len(input_total_part_cnt)):
        ...
        matched_pred_conf = pred_conf[t:t+total_cnt][matched_pred_ids]
        matched_pred_mask_conf_all.append(matched_pred_conf) 
        ...
        matched_mask_conf_loss = network.get_conf_loss(matched_pred_conf, matched_mask_loss)
        mask_conf_loss_per_data.append(matched_mask_conf_loss)

        t+= total_cnt
    matched_pred_mask_conf_all = torch.cat(matched_pred_mask_conf_all)
    matched_pred_mask_all = torch.cat(matched_pred_mask_all)
    matched_gt_mask_all = torch.cat(matched_gt_mask_all)
    mask_loss_per_data = torch.stack(mask_loss_per_data)
    mask_conf_loss_per_data = torch.cat(mask_conf_loss_per_data)
    
    # for each type of loss, compute avg loss per batch
    mask_loss = mask_loss_per_data.mean()
    conf_loss = mask_conf_loss_per_data.mean()
    # compute total loss
    total_loss = mask_loss * conf.loss_weight_mask 
                # conf_loss * conf.loss_weight_mask
    ...
    with torch.no_grad():
        ...
        if log_tb and tb_writer is not None:
            ...
            tb_writer.add_scalar('conf_loss', conf_loss.item(), step)
        ...
        if is_val and (not conf.no_visu) and epoch % conf.num_epoch_every_visu == 0:
            ...
            conf_mask_dir = os.path.join(out_dir, 'conf_mask')
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                for i in range(batch_size):
                    ...
                    cur_conf_mask = (matched_pred_mask_all[i].cpu().numpy() * matched_pred_mask_conf_all[i].cpu().numpy()).astype(np.uint8) * 255
                    Image.fromarray(cur_conf_mask).save(os.path.join(conf_mask_dir, fn))
                    ...
                    with open(os.path.join(info_dir, fn.replace('.png', '.txt')), 'w') as fout:
                        ...
                        fout.write('mask_loss: %f\n' % mask_loss_per_data.item())
                        fout.write('conf_loss: %f\n' % mask_conf_loss_per_data[i].item())  

                
            if batch_ind == conf.num_batch_every_visu - 1:
                # visu html
                utils.printout(conf.flog, 'Generating html visualization ...')
                sublist = 'input_img,input_pts,gt_mask,pred_mask,conf_mask,info'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf_alpha', type=float, default=0.5)
    ...
    parser.add_argument('--loss_weight_conf', type=float, default=0.2, help='loss weight')

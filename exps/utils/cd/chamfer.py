...
class ChamferDistanceFunctionIndex(torch.autograd.Function):
    @staticmethod
    def forward(ctx, xyz1, xyz2):
        xyz1 = xyz1.contiguous()
        xyz2 = xyz2.contiguous()
...
def chamfer_distance_idx(xyz1, xyz2, transpose=True, sqrt=False, eps=1e-12):
    """Chamfer distance
    ...
    if transpose:
        xyz1 = xyz1.transpose(1, 2)
        xyz2 = xyz2.transpose(1, 2)
    dist1, dist2, idx1, idx2 = ChamferDistanceFunctionIndex.apply(xyz1, xyz2)
    if sqrt:
        dist1 = safe_sqrt(dist1, eps)
        dist2 = safe_sqrt(dist2, eps)
    return dist1, dist2, idx1, idx2
